...
- Chrome 内部页面无法整理。
- 默认本地规则分类依赖标题、URL、域名和词表，无法完整理解网页正文语义。
- Service Worker 不是常驻后台。
- 当标签页标题过短或 URL 信息不足时，分类可能不准确。
- Chrome 内置 Gemini Nano 增强默认关闭，且只处理本地流程剩余的未分组标签，不会覆盖用户规则和内置词表结果。
- Chrome 内置 Gemini Nano 增强取决于 Chrome 版本、系统环境、地区、硬件、Prompt API 支持情况和模型下载状态。
- Chrome Prompt API 当前语言支持有限，本扩展对 Gemini Nano 使用英文分类 ID，再在本地映射为中文组名。
- Gemini Nano 返回结果仍需经过本地 schema 校验；校验失败时会自动回退到普通本地分类。

## 后续可扩展方向

- 更丰富的内置域名词表和负向关键词，减少常见站点误判。
- 支持用户编辑、启用或禁用内置分类。
- 为 Gemini Nano 增强模式增加可视化诊断，例如模型是否可用、是否需要下载、实际处理了哪些未分组标签。
- 引入本地 TF-IDF / cosine similarity 聚类，提升无需模型时的弱语义聚合效果。
- 支持从用户确认过的分组中学习本地偏好，但仍只保存在浏览器本地。
- 增加分类结果解释面板，展示用户规则、词表、关键词、聚类和 Gemini Nano 各自贡献。
